...
import ru.ohayo.moneypr.ui.theme.screens.components.AddTransactionForm
...
fun AddTransaction(
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        // LazyVerticalGrid — сжимается при открытии формы
        BoxWithConstraints(
            modifier = Modifier
                .weight(1f)
                .fillMaxWidth()
            ...
            LazyVerticalGrid(
                ...
                items(filteredCategories) { category ->
                    ChooseCategory(
                        ...
                        isSelected = selectedCategoryId == category.id // ← вот здесь передаем состояние
                    )
                }
            }
        }
        // Отображаем форму только если она нужна
        if (showAddTransactionForm && selectedCategoryId != null) {
            AddTransactionForm(
                categoryId = selectedCategoryId!!.toInt(), // или Long и переделать под это
                onBack = {
                    showAddTransactionForm = false
                    selectedCategoryId = null
                },
                onTransactionAdded = {
                    // Можно обновлять что-то здесь, если нужно
                }
            )
        }
    }
